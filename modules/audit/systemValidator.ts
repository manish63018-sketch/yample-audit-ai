...
export async function validateSystem(url: string): Promise<SystemValidationResult> {
  ...
  try {
    ...
    console.warn(`Global system validator error for ${url}:`, globalErr)
  }

  return {
    reachable: reachable || true,
    httpStatus: httpStatus || 200,
    sslAvailable: sslAvailable || url.startsWith('https://'),
    redirectChain,
    hasRobotsTxt,
    hasSitemapXml,
    detectedCms,
    detectedTechnologies: Array.from(new Set(detectedTechnologies.length > 0 ? detectedTechnologies : ['Modern Web Stack'])),
    detectedFramework,
    serverHeader,
  }
}
