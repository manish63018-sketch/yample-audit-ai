...
export async function POST(req: NextRequest) {
  try {
    ...
    // Demo mode — no real keys yet
    if (!keySecret || keySecret.includes('XXXXXXXX')) {
      ...
      return NextResponse.json({
        ...
        isDemo: true,
        redirectUrl: `/thankyou?payment=razorpay&id=${razorpay_payment_id}&demo=1`,
      })
    }
    ...
    try {
      ...
      await fetch(`${req.nextUrl.origin}/api/leads`, {
        ...
        body: JSON.stringify({
          ...
          email: customerEmail || '',
          services: Array.isArray(items) ? items.map((i: { name: string }) => i.name).join(', ') : '',
