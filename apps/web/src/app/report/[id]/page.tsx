...
function AuditReportContent({ auditId }: { auditId: string }) {
  ...
  useEffect(() => {
    ...
    const loadAuditData = async () => {
      ...
      if (typeof window !== 'undefined' && auditId) {
        const stored =
          sessionStorage.getItem(`audit_data_${auditId}`) ||
          sessionStorage.getItem(`audit_data_${cleanId}`);
        if (stored) {
          try {
  ...
  const fallbackUrl = searchUrl ? searchUrl.replace(/^https?:\/\//, '') : '';
  ...
  const healthBadge =
    overallScore >= 85
      ? {
          label: `Excellent (${overallScore}%)`,
          color: 'text-emerald-400',
          bg: 'bg-emerald-400',
          desc: 'Your website maintains strong performance and security foundations. Deploying AI automation will maximize conversion.',
        }
      : overallScore >= 65
      ? {
          label: `Needs Optimization (${overallScore}%)`,
          color: 'text-amber-400',
          bg: 'bg-amber-400',
          desc: 'Your website shows moderate technical health. Resolving performance and conversion bottlenecks will boost inquiry rates.',
        }
      : {
          label: `Critical Debt (${overallScore}%)`,
          color: 'text-red-400',
          bg: 'bg-red-400',
          desc: 'Your website demonstrates critical speed and conversion debt that is currently costing you search traffic and leads.',
        };
  ...
  const dynamicProblems = [
    ...
    ...(lcp && lcp > 2.5
      ? [
          {
            text: `Slow Mobile LCP Latency (${lcp}s) increases bounce rate`,
            impact: 'Mobile Visitor Loss',
          },
        ]
      : []),
    ...(imagesNoAlt > 0
      ? [
          {
            text: `${imagesNoAlt} images missing alt text attributes`,
            impact: 'SEO & WCAG Penalty',
          },
        ]
      : []),
    ...(!hasCsp
  ...
  const dynamicBeforeAfter = [
    ...
    },
    {
      ...
      before:
        missingFeats.length > 0 ? `Missing ${missingFeats.length} Lead Features` : 'Manual Intake',
  ...
  const aiRecommendations = auditData?.aiSummary?.recommendations || [
    {
      ...
      effort: 'medium',
      description:
        'Defer non-essential scripts, compress hero media assets, and enable CDN edge caching.',
    ...
    },
    {
      ...
      effort: 'low',
      description:
        'Integrate an automated AI agent to capture after-hours inquiries and answer FAQs.',
  ...
  return (
    <div className="min-h-screen bg-[#08080f] text-white print:bg-white print:text-black">
      ...
      <div className="border-b border-white/5 px-6 py-3.5 flex items-center justify-between max-w-7xl mx-auto print:hidden">
        ...
        >
          <Search className="w-4 h-4 text-violet-400" /> <span>AuditAI</span>{' '}
          <span className="text-xs text-violet-400 font-normal">by Yample Labs</span>
        </Link>
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/cart"
            className="px-3.5 py-1.5 rounded-xl bg-violet-500/10 text-violet-300 text-xs font-medium border border-violet-500/20 hover:bg-violet-500/20 transition-all flex items-center gap-1.5"
          >
        ...
        <div className="glass-card p-8 md:p-10 rounded-3xl border border-violet-500/30 bg-gradient-to-br from-violet-950/40 via-slate-900 to-slate-950 relative overflow-hidden">
          <div className="max-w-3xl space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-xs font-bold">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" /> Verified Audit
              Report for {url}
            </div>
            ...
            <p className="text-sm text-slate-300 leading-relaxed">
              {auditData?.aiSummary?.summary ||
                `We performed a technical scan and AI business analysis of ${url}. Here are the exact technical bottlenecks and revenue opportunities.`}
            </p>
          </div>
          ...
          <div className="mt-8 pt-6 border-t border-white/10 flex flex-wrap items-center gap-6">
            ...
            <p className="text-xs text-slate-400 max-w-md">{healthBadge.desc}</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="glass-card p-6 rounded-2xl border border-amber-500/20 bg-amber-500/5 text-center">
              <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                Estimated Organic Traffic Gain
              </div>
              <div className="text-4xl font-black text-amber-400 my-2">
                +{revenue.leadIncreasePercent}%
              </div>
              <div className="text-[11px] text-amber-300">
                Targeted SEO &amp; Core Web Vitals uplift
              </div>
            </div>
            ...
            <div className="glass-card p-6 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 text-center">
              ...
              <div className="text-4xl font-black text-emerald-400 my-2">
                ${revenue.estimatedMonthlyGainUsd.toLocaleString()}{' '}
                <span className="text-xs text-slate-500 font-normal">/ month</span>
              </div>
              <div className="text-[11px] text-emerald-300">
                Estimated incremental business revenue
              </div>
            </div>
            ...
            <div className="glass-card p-6 rounded-2xl border border-violet-500/20 bg-violet-500/5 text-center">
              ...
              <div className="text-4xl font-black text-violet-400 my-2">
                +{revenue.conversionUpliftPercent}%
              </div>
              <div className="text-[11px] text-violet-300">
                From AI lead widgets &amp; UX optimization
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-8 rounded-3xl border border-white/10 space-y-6">
          <div>
            ...
            <p className="text-slate-400 text-xs mt-1">
              Issues identified during automated crawl of {url}
            </p>
          </div>
        ...
        <div className="glass-card p-8 rounded-3xl border border-white/10 space-y-6">
          ...
          <div className="space-y-3">
            {dynamicBeforeAfter.map((item, idx) => (
              <div
                key={idx}
                className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-xl border border-white/5 bg-white/[0.02] text-xs"
              >
                <div className="font-bold text-white flex items-center">{item.feature}</div>
                <div className="text-red-300 bg-red-500/10 border border-red-500/20 p-2.5 rounded-lg">
                  {item.before}
                </div>
                <div className="text-emerald-300 bg-emerald-500/10 border border-emerald-500/20 p-2.5 rounded-lg font-semibold">
                  {item.after}
                </div>
              </div>
            ))}
        ...
        <div className="glass-card p-8 rounded-3xl border border-violet-500/30 bg-gradient-to-br from-violet-950/20 to-slate-900 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {aiRecommendations.map((rec: any, idx: number) => (
              <div
                key={idx}
                className="p-6 rounded-2xl border border-white/10 bg-white/5 space-y-3"
              >
        ...
        <div className="glass-card p-8 md:p-10 rounded-3xl border border-emerald-500/30 bg-gradient-to-r from-emerald-950/30 via-slate-900 to-violet-950/30 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="space-y-3 max-w-xl">
            ...
            <h3 className="text-2xl md:text-3xl font-black text-white">
              Full Growth &amp; Performance Overhaul
            </h3>
            <p className="text-xs text-slate-300 leading-relaxed">
              Includes Core Web Vitals optimization, SEO fixes, HSTS security headers, and 24/7 AI
              lead qualifier widget with 30-day warranty.
            </p>
          </div>
